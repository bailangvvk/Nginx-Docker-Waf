...
RUN \
  ...
  curl -v -fSL https://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz -o nginx.tar.gz || { echo "Failed to download nginx-${NGINX_VERSION}.tar.gz"; exit 1; } && \
  tar xzf nginx.tar.gz || { echo "Failed to extract nginx-${NGINX_VERSION}.tar.gz"; exit 1; } && \
  \
  curl -v -fSL https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz -o openssl.tar.gz || { echo "Failed to download openssl-${OPENSSL_VERSION}.tar.gz"; exit 1; } && \
  tar xzf openssl.tar.gz || { echo "Failed to extract openssl-${OPENSSL_VERSION}.tar.gz"; exit 1; } && \
  \
  curl -v -fSL https://fossies.org/linux/misc/zlib-${ZLIB_VERSION}.tar.gz -o zlib.tar.gz || { echo "Failed to download zlib-${ZLIB_VERSION}.tar.gz"; exit 1; } && \
  tar xzf zlib.tar.gz || { echo "Failed to extract zlib-${ZLIB_VERSION}.tar.gz"; exit 1; } && \
  \
  # 下载并解压 Brotli 和 Zstd
  curl -v -fSL https://github.com/google/brotli/archive/refs/tags/v${BROTLI_VERSION}.tar.gz -o brotli.tar.gz || { echo "Failed to download brotli-${BROTLI_VERSION}.tar.gz"; exit 1; } && \
  tar xzf brotli.tar.gz || { echo "Failed to extract brotli-${BROTLI_VERSION}.tar.gz"; exit 1; } && \
  \
  curl -v -fSL https://github.com/facebook/zstd/archive/refs/tags/v${ZSTD_VERSION}.tar.gz -o zstd.tar.gz || { echo "Failed to download zstd-${ZSTD_VERSION}.tar.gz"; exit 1; } && \
